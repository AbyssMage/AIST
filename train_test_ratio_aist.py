...
train_ratio = 0.5  # 0.5(6m), 0.5833(7m), 0.75(9m), 0.8333(10 m)
time_step = 120
batch_size = 42
target_region = d.tr # 65
target_cat = d.tc  # 3(starts from 0)
...
train_x_size = int(x.shape[0] * train_ratio)  # batch_size
# print(train_x_size, x.shape[0] - train_x_size)
# (1034 1035) -> 6m
# (1551, 518) -> 9m
...
sub_train = 26  # 26 (6m), 30 (7m), 0 (8m), 39(9m), 2(10m)
train_x = x[: train_x.shape[0] - sub_train, :]  # (batch_size, time-step) = (1386, 120)
train_x_daily = x_daily[: train_x_daily.shape[0] - sub_train, :]
train_x_weekly = x_weekly[: train_x_weekly.shape[0] - sub_train, :]
train_y = y[: train_y.shape[0] - sub_train, :]  # (batch_size, time-step) = (1386, 1)

sub_test = 27  # 27 (6m), 23 (7m), 11 (8m), 14(9m), 9(10m)
test_x = x[train_x_size:, :]  # (batch_size, time-step) = (683, 120)
test_x_daily = x_daily[train_x_size:, :]
test_x_weekly = x_weekly[train_x_size:, :]

test_x = test_x[:test_x.shape[0] - sub_test, :]  # (batch_size, time-step) = (672, 120) -- to make it consistent with the
test_x_daily = test_x_daily[:test_x_daily.shape[0] - sub_test, :]
test_x_weekly = test_x_weekly[:test_x_weekly.shape[0] - sub_test, :]
test_y = y[train_x_size:, :]  # (batch_size, time-step) = (683, 1)
test_y = test_y[:test_y.shape[0] - sub_test, :]

# print(train_x.shape, train_x_daily.shape, train_x_weekly.shape, train_y.shape)
...
train_feat, test_feat = load_data_regions_ratio(target_cat, target_region, train_ratio, sub_train, sub_test)
train_feat_ext, test_feat_ext = load_data_regions_external_v2_ratio(target_region, train_ratio, sub_train, sub_test)
train_crime_side, test_crime_side = load_data_sides_crime_ratio(target_cat, target_region, train_ratio, sub_train, sub_test)
...
shutil.copy('{}.pkl'.format(best_epoch), 'E:/aist_ratio/' + str(train_ratio) + '_' + str(target_region) + '_' + str(target_cat) + '_mae.pkl')
...
f = open('C:/Users/BD/PycharmProjects/AIST/aist_train_test_mae.txt','a')
...
def compute_test():
    ...
    print(train_ratio, " ", target_region, " ", target_cat, " ", loss/i)
    print(train_ratio, " ", target_region, " ", target_cat, " ", loss/i, file=f)
