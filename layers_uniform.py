...
class RNN_GRU(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, input_t in enumerate(x.chunk(timestep, dim=1)):
            ...
            input_t = input_t.contiguous().view(input_t.size()[0], input_t.size()[
                -1])  # input_t = (batch_size, 1, input_size) ---> (batch_size, input_size)
...
class RNN_LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, input_t in enumerate(x.chunk(x.size(1), dim=1)):
            # input_t = (batch_size, 1, input_size)
            input_t = input_t.contiguous().view(input_t.size()[0],
                                                input_t.size()[-1])  # input_t = (batch_size, input_size)
...
class self_LSTM_sparse_attn_predict(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, input_t in enumerate(x.chunk(time_size, dim=1)):
            ...
            # are left non-zero and the other ones are zeroed.
            #
            attn_w = torch.ones((batch_size, remember_size))
...
"""x_crime = torch.zeros(size=(42, 20, 8))
in_sab = self_LSTM_sparse_attn_predict_eval_interpre(1*8, 40, 1, 2, truncate_length=5, top_k=4, attn_every_k=5, predict_m=10)
in_sab(x_crime)"""
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj, ext_input, side_input):
        ...
        # Commented Oct23 11AM
        # a_input = torch.cat([h.repeat(1, 1, N).view(h.shape[0], N * N, -1),
        # h.repeat(1, N, 1)], dim=2).view(h.shape[0], N, -1, 2 * self.out_features)
        ...
        # a_input_side = torch.cat([h_side.repeat(1, 1, N).view(42, N * N, -1),
        # h_side.repeat(1, N, 1)], dim=2).view(42, N, -1, 2 * self.out_features)
        ...
        # Unifrom Attetnion Distribution - # Added 23 Oct - 11 AM
        attention = torch.ones((42, N, N))
        attention = F.softmax(attention, dim=2)  # shape = (B, N, N)
        # attention = F.dropout(attention, self.dropout, training=self.training)  # shape = (B, N, N)
        # print(attention[:, -1, :].mean(dim=0).view(1, -1).detach().numpy())
        ...
        # ext_input.repeat(1, N, 1, 1).view(ext_input.shape[0], N * N, n_feature, -1)], dim=3).view(
        # ext_input.shape[0], N, N, n_feature, 2)
        ...
        # Uniform Distribution - Added 23 Oct - 11 AM
        dot_attention = torch.ones((42, N, N, n_feature))
        dot_attention = F.softmax(dot_attention, dim=3)  # shape = (B, N, N, nfeat)
        # print(dot_attention.mean(dim=0)[-1].sum(dim=0).detach().softmax(dim=0).view(1, -1).numpy())
