...
class Temporal_Module(nn.Module):
    ...
    def forward(self, x_crime, x_crime_daily, x_crime_weekly, x_regions, x_sp_crime, x_ext, s_crime):
        ...
        # print(x_con_attn.view(42, -1).softmax(dim=1).mean(dim=0).view(1, -1).detach().numpy())
        ...
        # print(x_daily_attn.view(42, -1).softmax(dim=1).mean(dim=0).view(1, -1).detach().numpy())
        ...
        # print(x_weekly_attn.view(42, -1).softmax(dim=1).mean(dim=0).view(1, -1).detach().numpy())
        ...
        # Uniform Distribution Added 23 Oct
        alpha_m = torch.ones(42, 3)
        alpha_m = torch.softmax(alpha_m, dim=1)
        attn_trend = alpha_m.detach()
        # print(alpha_m.view(42, -1).mean(dim=0).view(1, -1).detach().numpy())
...
class Spatial_Module(nn.Module):
    ...
    def forward(self, x_crime, x_regions, x_ext, s_crime):
        ...
        for i in range(100, 120):
            batch_output = []
            for j in range(B):  # take each sample
                np.savetxt("gat_feat.txt", feat[i][j], fmt='%d')
                np.savetxt("gat_feat_ext.txt", feat_ext[i][j], fmt='%d')
                np.savetxt("gat_crime_side.txt", crime_side[i][j], fmt='%d')
                adj, features, features_ext, crime_side_features = load_data_GAT()
                out, ext = self.gat(features, adj, features_ext, crime_side_features)  # (N, F')(N, N, dv)
                out = out.view(N, out.shape[1])
                out = out[-1, :]
                # ext = ext.view(N, ext.shape[1])
                # ext = ext[-1, :]
                # out = torch.stack((out, ext), dim=2)
                batch_output.append(out)
            batch_output = torch.stack(batch_output, 0)
            spatial_output.append(batch_output)
