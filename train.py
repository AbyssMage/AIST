...
loaded_data = torch.from_numpy(np.loadtxt("data/" + location + "/com_crime/r_" + str(target_region) + ".txt", dtype=np.int)).T
...
train_nei, test_nei = load_nei_crime(target_cat, target_region, location)
train_ext, test_ext = load_all_ext(target_region, location)
train_side, test_side = load_all_parent_crime(target_cat, target_region, location)

# Model and optimizer
model = AIST(in_hgat, in_fgat, out_gat, att_dot, nhid_rnn, nlayer_rnn, att_rnn, ts, target_region, target_cat, location)
