    ...
    def forward(self, x):
        ...
        for i, input_t in enumerate(x.chunk(time_size, dim=1)):
            ...
            for i, input_t in enumerate(x.chunk(time_size, dim=1)):
                ...
                mlp_h_attn = torch.cat((h_repeated, h_old),
                                       2)  # mlp_h_attn = (batch_size, remember_size, 2* hidden_size)
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj, ext_input, side_input):
        ...
        r_att_arr = attention[:, -1, :]  # attention of each instance
        ...
        f_att_arr = dot_attention[:, -1, :, :].sum(dim=1).softmax(dim=1)
        ...
        if self.concat:
            return F.elu(h_prime), F.elu(ext_rep), r_att_arr, f_att_arr
        else:
            return h_prime, ext_rep, r_att_arr, f_att_arr
